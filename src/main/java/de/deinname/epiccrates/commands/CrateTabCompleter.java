...
public final class CrateTabCompleter implements TabCompleter {
    ...
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if ((args[0].equalsIgnoreCase("give") && (args.length == 2 || args.length == 3)) || (args[0].equalsIgnoreCase("preview") && args.length == 2) || (args[0].equalsIgnoreCase("key") && args.length == 4)) return Arrays.stream(CrateType.values()).map(CrateType::id).filter(value -> value.startsWith(args[args.length - 1].toLowerCase())).toList();
